#!/bin/bash
# Dracut module: support booting SuperISO as a file from Ventoy-style media.
...
installkernel() {
    ...
    instmods exfat vfat fat ntfs3 ext4 btrfs nls_cp437 nls_iso8859_1 nls_utf8
}

install() {
    inst_multiple mount umount blkid losetup find grep sed mkdir ln readlink basename cat sleep udevadm
    inst_hook initqueue 20 "$moddir/superiso-isofile.sh"
}
