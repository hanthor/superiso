#!/usr/bin/bash
# End-to-end composefs install test (WITHOUT encryption) for a built SuperISO.
# Usage: scripts/test-fisherman-composefs-install.sh <iso> [ssh-port] [fisherman-binary]
# This validates that composefs backend DOES bypass bootupd requirement.
...
RECIPE=$(mktemp /tmp/fisherman-composefs-recipe.XXXXXX.json)
...
jq -n \
    ...
    '{
      ...
      hostname:"superiso-composefs-ci",
