...
set +e
sshpass -p live ssh "${ssh_base[@]}" -p "$PORT" liveuser@127.0.0.1 \
    "export SUPERISO_LUKS_PASSPHRASE=${PASSPHRASE_Q} SUPERISO_LUKS_BOOTLOADER=${BOOTLOADER_Q} SUPERISO_LUKS_COMPOSEFS=${COMPOSEFS_OVERRIDE_Q} SUPERISO_LUKS_SELINUX_DISABLED=${SELINUX_DISABLED_Q}; bash -x -s" <<'REMOTE'
set -euo pipefail
